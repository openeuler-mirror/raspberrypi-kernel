				  ...
				  enum stage_group stage)
{
	...
	if (stage_is_bio(hstage->stage)) {
		...
		WARN(!list_empty(&bio_ddata->head),
		     ...
		     kobject_name(q->mq_kobj->parent), hierarchy_stage_name(stage));
	}
...
void hierarchy_add_bio(struct hierarchy_stage *hstage, struct bio *bio)
{
	...
	list_add_tail(&bio->hierarchy_list, &bio_ddata->head);
...
void hierarchy_remove_bio(struct hierarchy_stage *hstage, struct bio *bio)
{
	...
	spin_lock_irqsave(&bio_ddata->lock, flags);
	list_del_init(&bio->hierarchy_list);
	spin_unlock_irqrestore(&bio_ddata->lock, flags);
}
...
static void __hierarchy_show_bio(struct seq_file *m, struct bio *bio,
				 enum stage_group stage, u64 duration)
{
	...
	blk_fill_rwbs(rwbs, bio->bi_opf);
	blkg_path(bio->bi_blkg, path, PATH_LEN);

	seq_printf(m, "%s-%d %s stage %s bio %s %llu + %u cgroup %s started %llu ns ago\n",
		   task ? task->comm : "null", task ? task->pid : 0,
		   bio->bi_bdev->bd_disk->disk_name,
		   hierarchy_stage_name(stage), rwbs, bio->bi_iter.bi_sector,
		   bio_sectors(bio), path, duration);
...
static void hierarchy_show_bio(struct seq_file *m, struct bio *bio)
{
	u64 duration;
	struct hierarchy_stage *hstage = m->private;
	struct bio_dump_data *bio_ddata = hstage->dump_data;

	duration = get_duration(bio_ddata->stat_time, bio->hierarchy_time);
	if (hstage->threshold > ns_to_ms(duration))
		return;

	__hierarchy_show_bio(m, bio, hstage->stage, duration);
}

static int bio_hierarchy_list_show(struct seq_file *m, void *v)
{
	struct bio *bio = list_entry(v, struct bio, hierarchy_list);

	hierarchy_show_bio(m, bio);
	return 0;
}
...
static void rq_hierarchy_init_dump_data(struct rq_dump_data *rq_ddata)
{
	...
	rq_ddata->shared = blk_mq_is_shared_tags(q->tag_set->flags);
	rq_ddata->has_elevator = !!q->elevator;

	if (rq_ddata->shared)
		rq_ddata->total_tags = rq_ddata->has_elevator ?
			q->nr_requests :
			q->tag_set->shared_tags->nr_tags;
...
static struct request *hierarchy_find_and_get_rq(struct rq_dump_data *rq_ddata)
{
	...
	if (nr_tag >= rq_ddata->total_tags) {
		...
		hctx = xa_load(&q->hctx_table, hctx_id);
		rq = hctx->fq->flush_rq;
	} else if (rq_ddata->shared) {
		struct blk_mq_tags *tags = rq_ddata->has_elevator ?
			q->sched_shared_tags : q->tag_set->shared_tags;

		rq = tags->static_rqs[nr_tag];
	} else if (rq_ddata->has_elevator) {
...
static int bio_stage_hierarchy_show(struct seq_file *m, void *v)
{
	...
	if (stage_is_bio(pdata->stage)) {
		...
		duration = get_duration(bstage_ddata->stat_time,
					bio->bi_alloc_time_ns);
		if (hstage->threshold <= ns_to_ms(duration))
			__hierarchy_show_bio(m, bio, pdata->stage, duration);
