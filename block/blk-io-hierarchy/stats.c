...
void bio_hierarchy_start_io_acct(struct bio *bio, enum stage_group stage)
{
	...
	io_hierarchy_inc(hstage->hstats_data, dispatched, bio_hierarchy_op(bio));
				 ...
				 u64 time)
{
	...
	duration = time - bio->hierarchy_time;
	hstage = q->io_hierarchy_stats->hstage[stage];
					 ...
					 enum stage_group new_stage)
{
	...
	pr_warn("blk-io-hierarchy: disk %s stage %d(%s) -> %d(%s) unbalanced accounting.",
		kobject_name(rq->q->mq_kobj->parent),
				...
				struct hierarchy_stage *hstage)
{
	...
	if (rq->stage != hstage->stage) {
		rq_hierarchy_warn_unbalanced(rq, hstage, rq->stage,
					     hstage->stage);
		return;
	}

	op = rq_hierarchy_op(rq);
	duration = jiffies - rq->hierarchy_time;
